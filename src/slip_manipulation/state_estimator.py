...
class StateEstimator():
    ...
    def estimate_contact_config(self, contact=True):
        ...
        for col in range(box_rot_mat.shape[1]):
            ...
            angle = np.arccos(np.dot(box_axis, base_z_axis) / (np.abs(box_axis) * np.abs(base_z_axis)))
            angles.append(angle)
        ...
        if np.isclose(min_angle_deg, 0, atol=5):
    ...
    def vision_estimate_contact(self):
        ...
        coords = []
        coords.append((self.box_dim[0], self.box_dim[1], self.box_dim[2]))
        coords.append((self.box_dim[0], self.box_dim[1], -self.box_dim[2]))
        coords.append((self.box_dim[0], -self.box_dim[1], self.box_dim[2]))
        coords.append((self.box_dim[0], -self.box_dim[1], -self.box_dim[2]))
        coords.append((-self.box_dim[0], self.box_dim[1], self.box_dim[2]))
        ...
        z_height = []
        for coord in coords:
            vertex = Pose(coord, (0, 0, 0, 1))
            base_vertex = tf_transform_pose(self.tf_buffer, vertex, 'box_origin', 'base_link').pose.position
            z_height.append(base_vertex.z)
