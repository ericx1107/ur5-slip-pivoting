...
class BoxMarkers():
    def __init__(self, box_dim, grasp_param=1, detect_once=False):
        ...
        self.markers_sub = rospy.Subscriber('/fiducial_transforms', 
            FiducialTransformArray, self.marker_callback)

        self.markers_pose_pub = rospy.Publisher('/slip_manipulation/marker_pose', PoseStamped, queue_size=10)
        
        self.visualise_box_pub = rospy.Publisher('/slip_manipulation/box_visualisation', Marker, queue_size=10)
        
        self.grasp_pub = rospy.Publisher('/slip_manipulation/grasp_pose', PoseStamped, queue_size=10)
        
        self.long_pub = rospy.Publisher('/slip_manipulation/is_long_edge', Bool, queue_size=10)
    ...
    def marker_callback(self, marker_array):
        for marker in marker_array.transforms:
            # self.marker_list[str(marker.id)]["pose"] = marker.pose.pose
            
            # construct stamped message
            stamped = PoseStamped()
            stamped.header = marker_array.header
            stamped.pose.position.x = marker.transform.translation.x
            stamped.pose.position.y = marker.transform.translation.y
            stamped.pose.position.z = marker.transform.translation.z
            stamped.pose.orientation = marker.transform.rotation

            # publish marker pose
            self.markers_pose_pub.publish(stamped)

            # publish tf's
            self.publish_marker_transform(stamped, marker.fiducial_id)
            self.publish_box_origin_transform(marker.fiducial_id)
